...
function installVercel(pkgManager) {
  ...
  try { const r = spawnSync(entry[0], entry[1], { stdio: 'inherit', shell: isWindows }); if (r.status !== 0) throw new Error(`Exit code: ${r.status}`); } catch (e) { log(`Installation failed: ${e.message}`); process.exit(1); }
}
function main() {
